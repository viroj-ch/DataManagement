...
namespace DataManagement.Repository
{
    public class CustomerRepository : BaseRepository, IRepository<Customer>
    {
        ...
        {
            IList<Customer> customerList = SqlMapper.Query<Customer>(con, "GetAllCustomers", commandType: StoredProcedure).ToList();
            return customerList;
        }
        public Customer Get(int id)
        {
            DynamicParameters parameters = new DynamicParameters();
            parameters.Add("@CustomerID", id);
            return SqlMapper.Query<Customer>((SqlConnection)con, "GetCustomerById", parameters, commandType: StoredProcedure).FirstOrDefault();
        }
